...
class Admin {
	...
	public function register_hooks() {
		...
		add_action( 'admin_post_furmrowi_save_settings', array( $this, 'save_settings' ) );
		add_action( 'admin_post_furmrowi_admin_evidence', array( $this, 'download_evidence' ) );
	}
	...
	public function enqueue_assets() {
		...
		if ( in_array( $page, array( 'furmrowi-withdrawals', 'furmrowi-settings' ), true ) ) {
			wp_enqueue_style( 'furmrowi-admin', FURMROWI_URL . 'assets/css/admin.css', array(), FURMROWI_VERSION );
		}
	}
		...
		<div class="wrap furmrowi-admin">
			...
			<table class="widefat striped">
				...
				<?php foreach ( $rows as $row ) : ?>
					<tr>
						<td><a href="<?php echo esc_url( add_query_arg( array( 'page' => 'furmrowi-withdrawals', 'view' => (int) $row['withdrawal_id'] ), admin_url( 'admin.php' ) ) ); ?>"><strong><?php echo esc_html( $row['reference'] ); ?></strong></a></td>
	...
	private function detail_page( $withdrawal_id ) {
		...
		$download_url = wp_nonce_url(
			add_query_arg( array( 'action' => 'furmrowi_admin_evidence', 'withdrawal_id' => (int) $withdrawal_id ), admin_url( 'admin-post.php' ) ),
			'furmrowi_admin_evidence_' . (int) $withdrawal_id
		);
		?>
		<div class="wrap furmrowi-admin">
			<h1><?php echo esc_html( $record['reference'] ); ?></h1>
			<p><a href="<?php echo esc_url( add_query_arg( 'page', 'furmrowi-withdrawals', admin_url( 'admin.php' ) ) ); ?>">&larr; <?php esc_html_e( 'Back to declarations', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></a></p>
			<div class="furmrowi-card"><h2><?php esc_html_e( 'Submission details', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2>
			...
			<div class="furmrowi-card"><h2><?php esc_html_e( 'Immutable declaration', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2><pre><?php echo esc_html( $record['declaration'] ); ?></pre><p><a class="button" href="<?php echo esc_url( $download_url ); ?>"><?php esc_html_e( 'Download evidence', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></a></p></div>
		</div>
		<?php
	}
	...
	public function settings_page() {
		...
		<div class="wrap furmrowi-admin">
			...
			<form method="post" action="<?php echo esc_url( admin_url( 'admin-post.php' ) ); ?>">
				<input type="hidden" name="action" value="furmrowi_save_settings">
				<?php wp_nonce_field( 'furmrowi_save_settings' ); ?>
				<div class="furmrowi-card"><h2><?php esc_html_e( 'Availability', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2>
				...
				<div class="furmrowi-card"><h2><?php esc_html_e( 'Merchant details', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2>
				...
				<div class="furmrowi-card"><h2><?php esc_html_e( 'Withdrawal information', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2>
				...
				<div class="furmrowi-card"><h2><?php esc_html_e( 'Pages and privacy', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2>
				...
				<div class="furmrowi-card"><h2><?php esc_html_e( 'Placement tools', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></h2><p><code>[furmrowi_form]</code> <code>[furmrowi_legal_notice]</code> <code>[furmrowi_link]</code></p><p><?php esc_html_e( 'The same form, legal notice and link are available as Gutenberg blocks and classic widgets.', 'furmedia-romanian-withdrawal-law-for-woocommerce' ); ?></p></div>
				<?php submit_button(); ?>
			</form>
		</div>
		<?php
	...
	public function save_settings() {
		$this->authorize();
		check_admin_referer( 'furmrowi_save_settings' );
		$input           = array();
		...
		wp_safe_redirect( add_query_arg( array( 'page' => 'furmrowi-settings', 'updated' => '1' ), admin_url( 'admin.php' ) ) );
		exit;
	}
	...
	public function download_evidence() {
		...
		check_admin_referer( 'furmrowi_admin_evidence_' . $id );
	...
	private function page_select( $name, $label, $selected ) {
		...
		$dropdown  = wp_dropdown_pages(
			array(
				...
				'id'                => esc_attr( 'furmrowi-' . $safe_name ),
		...
		echo '<p><label for="' . esc_attr( 'furmrowi-' . $safe_name ) . '">' . esc_html( $label ) . '</label> ';
